...
## 6.3. Implement app2 dns networkpolicies
...
kind: NetworkPolicy
metadata:
  name: default.ingress-egress-to-and-from-app2
  namespace: app2
spec:
  tier: default
  order: 200
...
```
APP2_POD=$(kubectl get pod -n app2 --no-headers -o name | head -1) && echo $APP2_POD
```
```
kubectl exec -it $APP2_POD -n app2 -- sh
```
